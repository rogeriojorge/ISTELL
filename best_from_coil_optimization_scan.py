...
results_path = os.path.join(os.path.dirname(__file__), 'results_'+QA_or_QH+'_nfp3' if use_nfp3 else '','scan')
...
if QA_or_QH == 'QH':
    ...
    succeeded = np.logical_and(succeeded, df["Jf"] < 1e-4)
    ...
    succeeded = np.logical_and(succeeded, df["Jf"] < 1e-5)
    succeeded = np.logical_and(succeeded, df["max_max_curvature"] < 10)
    succeeded = np.logical_and(succeeded, df["coil_surface_distance1"] > 0.049)
    succeeded = np.logical_and(succeeded, df["coil_surface_distance2"] > 0.049)
    succeeded = np.logical_and(succeeded, df["length"] < 17)
...
fields = (
    ...
    ("cc_threshold", False),
    ("cc_weight", True),
    ("coil_surface_distance1", False),
    ("coil_surface_distance2", False),
)
